...
3. **Заполнение анкеты:**
   - Респондент заполняет анкету на коллегу.
...
5. **Уведомление HR**  
   Бот отправляет сообщение HR-админу  
   ```
   Цикл: {TargetNick}
   ...
   ```
   Если все анкеты собраны — бот добавляет кнопку **🟢 Завершить сейчас**  
   (эквивалент `/finish <Cycle_ID>`).
